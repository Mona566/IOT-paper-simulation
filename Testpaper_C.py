    ...
    for mc in range(MONTE_CARLOS):
        ...
        for i in range(TimeSteps):
            ...
            for NodeID in range(NUM_NODES):
                ...
                Node_Rep[i,NodeID]=Net.Nodes[4].Rep[NodeID]
        ...
        #Somenodes=+Net.Nodes[3].Neighbours+Net.Nodes[4].Neighbours
        SomeNodeID=[3,4]
...
#                                      '\nquantum = ' + str(QUANTUM) +
    ...
    np.savetxt(dirstr+'/All_Noderep.csv', All_Noderep, delimiter=',')
...
def plot_results(dirstr):
    ...
    for NodeID in range(NUM_NODES):
        ...
        if MODE[NodeID]==1:
            ...
            ax1[1].plot(np.arange(window, SIM_TIME, STEP), avgTP[int(window/STEP):,NodeID]*Sum_noderep/(NU*Node_Rep[int(window/STEP):,NodeID]), linewidth=5*Node_Rep[0,NodeID]/Node_Rep[0,0], color='tab:blue', marker=marker, markevery=0.1)
        if MODE[NodeID]==2:
            ...
            ax1[1].plot(np.arange(window, SIM_TIME, STEP), avgTP[int(window/STEP):,NodeID]*Sum_noderep/(NU*Node_Rep[int(window/STEP):,NodeID]), linewidth=5*Node_Rep[0,NodeID]/Node_Rep[0,0], color='tab:red', marker=marker, markevery=0.1)
        if MODE[NodeID]>2:
            ...
            ax1[1].plot(np.arange(window, SIM_TIME, STEP), avgTP[int(window/STEP):,NodeID]*Sum_noderep/(NU*Node_Rep[int(window/STEP):,NodeID]), linewidth=5*Node_Rep[0,NodeID]/Node_Rep[0,0], color='tab:green', marker=marker, markevery=0.1)
            mal = True
    if mal:
    ...
    for NodeID in range(NUM_NODES):
        if MODE[NodeID]==0:
            ax6.bar(NodeID, Node_Rep[int(SIM_TIME/STEP)-1, NodeID], color='gray')
        if MODE[NodeID]==1:
            ax6.bar(NodeID, Node_Rep[int(SIM_TIME/STEP)-1, NodeID], color='tab:blue')
        if MODE[NodeID]==2:
            ax6.bar(NodeID, Node_Rep[int(SIM_TIME/STEP)-1, NodeID], color='tab:red')
        if MODE[NodeID]>2:
            ax6.bar(NodeID, Node_Rep[int(SIM_TIME/STEP)-1, NodeID], color='tab:green')
...
class Node:
    ...
    def __init__(self, Network, NodeID, Genesis, PoWDelay = 1):
        ...
        self.TranCounter = 0
        self.Blacklist = []
        self.Quantum=[MAX_WORK*rep/sum(self.Rep) for rep in self.Rep]
        self.Repchange=True
    ...
    def set_rate(self, Time):
        ...
        if MODE[self.NodeID]>0:
            if MODE[self.NodeID]==2 and Time>=START_TIMES[self.NodeID]: # AIMD starts after settling time
                ...
                self.Lambda = NUM_NEIGHBOURS*NU*self.Rep[self.NodeID]/sum(self.Rep)
    ...
    def add_to_inbox(self, Packet, Time):
        ...
        if Tran not in self.Inbox.Trans:
            if Tran not in self.Ledger:
                NodeID = Tran.NodeID
                ScaledWork = self.Inbox.Work[NodeID]/self.Rep[self.NodeID]
                    ...
                    if NodeID not in self.Blacklist:
                        if NodeID==self.NodeID:
                            ...
                            if ScaledWork>DROP_TH and BLACKLIST:
                                ...
                                print('Blacklisting NodeID', NodeID)
                            else:
        ...
        while self.Old:
            ...
            if self.Packets[NodeID]:
                ...
                if self.Deficit[NodeID]>=Work:
                    ...
                    self.Old.append(NodeID)
                    self.Deficit[self.Old[0]] += self.Quantum[self.Old[0]]
            else:
                self.Old.remove(NodeID)
...
class CommChannel:
    ...
    def deliver_packet(self, Packet, Time):
        ...
        if isinstance(Packet.Data, Transaction):
            ...
            # if Packet.Data.Rep_massage and self.TxNode.NodeID==1 and self.TxNode.Change:
            #     self.TxNode.Change=False
            #     self.TxNode.Rep[self.TxNode.NodeID]-=Packet.Data.Rep_change
            #     for q in self.TxNode.Neighbours:
            #         self.TxNode.Rep[q.NodeID]+=Packet.Data.Rep_change/4
            #     for q in self.TxNode.Neighbours:
            #         q.Rep=self.TxNode.Rep
            # if Packet.Data.Rep_massage and (self.TxNode.NodeID==3 or self.TxNode.NodeID==4):
            #     self.TxNode.Change=False
            #     self.TxNode.Rep[self.TxNode.NodeID]-=Packet.Data.Rep_change
            #     for q in self.TxNode.Neighbours:
            #         self.TxNode.Rep[q.NodeID]+=Packet.Data.Rep_change/4
